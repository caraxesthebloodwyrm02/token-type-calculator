...
def expecto_patronum(exchange: Exchange, anchor: BondMemory) -> ProtectiveForm | None:
    ...
    if anchor.bond_dynamics != "static":
        return None
    if exchange.engagement_cost < 1.0:
        return None
    if exchange.service_value < 1.0:
        return None
...
def check_freedom_signal(active_tokens: set, exchange: Exchange) -> bool:
    ...
    """
    return 'freedom-signal' in active_tokens and exchange.engagement_cost >= 0.8
...
class TokenTypeCalculator:
    ...
    def compute(self) -> dict[str, Any]:
        ...
        exchange_obj = Exchange(
            engagement_cost=self.params['engagement_cost'],
            service_value=self.params['service_value']
        )
